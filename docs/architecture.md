...
Factory-computer applications are owner-scoped registrations. The runtime
clones a live vanilla launcher entry only while the computer UI exists and
routes its button press back to the owning mod. Persistent employee queries and
assignments bridge `EmployeeManager`; writes require an active local Mirror
server and preserve vanilla offsite-contract exclusivity.
